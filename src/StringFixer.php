...
class StringFixer
{
    ...
    protected bool $wasPrefixed;
    protected array $appliedFixers = [];
    ...
    protected function __construct(string $original)
    {
        $this->wasPrefixed = str_starts_with($original, '<?');
        $this->original = $this->wasPrefixed ? $original: '<?php '.$original;
    }

    public function __toString(): string
    {
        $candidate = $this->new ?: $this->original;

        if ($this->wasPrefixed) {
            return $candidate;
        }

        return ltrim(substr($candidate, strlen('<?php')));
    }
    ...
    public function fix(): self
    {
        ...
        $tokens = Tokens::fromCode($this->original);
        ...
        foreach ($this->fixers() as $fixer) {
            ...
            if ($tokens->isChanged()) {
                ...
                $tokens->clearChanged();
                $this->appliedFixers[] = $fixer->getName();
            }
        }

        $this->new = $tokens->generateCode();

        return $this;
    }

    public function appliedFixers(): array
    {
    ...
    {
        try {
            (new TokenizerLinter)->lintSource($this->original)->check();
    ...
    protected function fixers(): array
    {
        ...
        return $fixerFactory
            ->useRuleSet(new RuleSet($this->rules()))
